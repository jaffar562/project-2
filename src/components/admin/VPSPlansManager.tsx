...
import { Plus, Edit, Trash2, Save, X, Server, Shield, Zap } from 'lucide-react';
import { useForm } from 'react-hook-form';
import toast from 'react-hot-toast';
import { supabase, VPSPlan } from '../../lib/supabase';

interface VPSPlansManagerProps {
  theme?: string;
}

const VPSPlansManager: React.FC<VPSPlansManagerProps> = ({ theme = 'dark' }) => {
  ...
  const fetchPlans = async () => {
    try {
      ...
      toast.error('Failed to fetch VPS plans');
  ...
  const handleAddPlan = async (data: any) => {
    try {
      ...
      toast.success('VPS plan added successfully');
      setShowAddForm(false);
      reset();
      fetchPlans();
    } catch (error) {
      toast.error('Failed to add VPS plan');
      console.error('Error:', error);
    }
  };
  ...
  const handleUpdatePlan = async (id: string, data: any) => {
    try {
      ...
      toast.success('VPS plan updated successfully');
      setEditingPlan(null);
      fetchPlans();
    } catch (error) {
      toast.error('Failed to update VPS plan');
      console.error('Error:', error);
    }
  };
  ...
  const handleDeletePlan = async (id: string) => {
    ...
    try {
      ...
      toast.success('VPS plan deleted successfully');
      fetchPlans();
    } catch (error) {
      toast.error('Failed to delete VPS plan');
      console.error('Error:', error);
    }
  };
  ...
  const togglePlanStatus = async (id: string, currentStatus: boolean) => {
    try {
      ...
      toast.success(`VPS plan ${!currentStatus ? 'activated' : 'deactivated'} successfully`);
      fetchPlans();
    } catch (error) {
      toast.error('Failed to update VPS plan status');
      console.error('Error:', error);
    }
  };
  ...
  const PlanForm = ({ plan, onSubmit, onCancel }: any) => (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label className={`block text-sm font-medium ${themeStyles.textSecondary} mb-1`}>Bandwidth</label>
          <input
            {...register('bandwidth', { required: 'Bandwidth is required' })}
            defaultValue={plan?.bandwidth}
            className={`w-full px-3 py-2 ${themeStyles.input} border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500`}
            placeholder="1TB"
          />
          {errors.bandwidth && <p className="text-red-400 text-sm mt-1">{errors.bandwidth.message as string}</p>}
        </div>

        <div>
          <label className={`block text-sm font-medium ${themeStyles.textSecondary} mb-1`}>Processor</label>
          <input
            {...register('processor')}
            defaultValue={plan?.processor}
            className={`w-full px-3 py-2 ${themeStyles.input} border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500`}
            placeholder="V4 Processor"
          />
        </div>

        <div>
          <label className={`block text-sm font-medium ${themeStyles.textSecondary} mb-1`}>Network</label>
          <input
            {...register('network')}
            defaultValue={plan?.network}
            className={`w-full px-3 py-2 ${themeStyles.input} border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500`}
      ...
      <div>
        ...
        <input
          ...
          className={`w-full px-3 py-2 ${themeStyles.input} border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500`}
          placeholder="Basic DDoS Protection, Root Access, Instant Setup"
        />
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className={`text-3xl font-bold ${themeStyles.text}`}>VPS Plans Manager</h1>
          <p className={`${themeStyles.textSecondary} mt-1`}>Manage your VPS hosting plans and pricing</p>
        </div>
        <button
      ...
      {showAddForm && (
        <div className={`${themeStyles.card} border rounded-xl p-6`}>
          <h3 className={`text-lg font-semibold ${themeStyles.text} mb-4`}>Add New VPS Plan</h3>
          <PlanForm
                ...
                <div className="space-y-2 mb-4">
                  <div className="flex justify-between">
                    ...
                    <span className={`font-semibold ${themeStyles.text}`}>
                      ₹{plan.price_inr}/mo
                      {plan.price_usd && <span className="text-xs ml-1">(${plan.price_usd})</span>}
                    </span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    <span className={`text-sm ${themeStyles.textMuted}`}>Bandwidth:</span>
                    <span className={`${themeStyles.textSecondary}`}>{plan.bandwidth}</span>
                  </div>
                  {plan.processor && (
                    <div className="flex justify-between">
                      <span className={`text-sm ${themeStyles.textMuted}`}>Processor:</span>
                      <span className={`${themeStyles.textSecondary}`}>{plan.processor}</span>
                    </div>
                  )}
                  {plan.network && (
                    <div className="flex justify-between">
                      <span className={`text-sm ${themeStyles.textMuted}`}>Network:</span>
                      <span className={`${themeStyles.textSecondary}`}>{plan.network}</span>
                    </div>
                  )}
                </div>
      ...
      {plans.length === 0 && (
        <div className={`${themeStyles.card} border rounded-xl p-12 text-center`}>
          ...
          <h3 className={`text-lg font-semibold ${themeStyles.text} mb-2`}>No VPS plans found</h3>
          <p className={`${themeStyles.textSecondary} mb-4`}>Get started by adding your first VPS plan.</p>
          <button
            onClick={() => setShowAddForm(true)}
...
export default VPSPlansManager;